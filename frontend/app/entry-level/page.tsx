  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ Filter hanya produk dengan category_id = 1
        const entryProducts = allProducts.filter(
          (p: any) => Number(p.category_id) === 1
        );

        setProducts(entryProducts);
      } catch (err) {
        console.error("❌ Gagal memuat produk Entry Level:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className={`${orbitron.variable} font-sans bg-white text-black`}>
      ...
      <section className="text-center py-20 px-6 border-b border-gray-200 pt-40">
        ...
        <p className="text-gray-700 max-w-3xl mx-auto text-[17px] leading-relaxed">
          Discover Chronova’s Entry Level timepieces — the perfect introduction 
          to our world of craftsmanship, precision, and timeless design.
          Accessible luxury for those beginning their journey into horology.
        </p>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        {loading ? (
          ...
          <div className="col-span-full text-center text-gray-400 py-20">
            No Entry Level products found (category_id = 1).
          </div>
        ) : (
          products.slice(0, 3).map((p, i) => (
            <motion.div
              ...
              onClick={() => router.push(`/product/${p.id}`)} // ✅ redirect ke product detail
      ...
      <section className="relative w-full h-[100vh] overflow-hidden">
        ...
        >
          <source
            src="https://www.tagheuer.com/assets/videos/2025/LP/Collection/Formula-1/Date/tag-heuer-formula-one-solargraph-38mm-16-9.mp4"
            type="video/mp4"
          />
        </video>
        <div className="absolute inset-0 bg-black/40 flex flex-col justify-center items-center text-white">
          ...
          <p className="text-lg max-w-2xl text-gray-200 text-center">
            Each Chronova Entry timepiece is crafted for precision, 
            designed for those who value subtle sophistication.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto py-24 px-6 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p className="text-gray-600 mb-6">
            Each Entry Level watch carries Chronova’s DNA — a balance between
            precision and elegance. Crafted to accompany you on every occasion.
          </p>
          <Link
            href="/products"
        ...
        <div>
          ...
          >
            <source
              src="https://www.tagheuer.com/assets/videos/2025/PLP/Collections/TAG_Heuer_Formula_1/TAGHeuer-plp-merchandize-portrait-f1-solargraph-case-video.mp4"
              type="video/mp4"
            />
          </video>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        {products.slice(3, 6).map((p, i) => (
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.2 }}
            onClick={() => router.push(`/product/${p.id}`)} // ✅ clickable
            className="group bg-[#fdfdfd] border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-500 cursor-pointer"
          >
            <div className="relative overflow-hidden bg-[#fafafa]">
              <img
      ...
      <section className="text-center py-24 border-t border-gray-200">
        ...
        <div className="max-w-md mx-auto">
          ...
          <h3 className="text-2xl font-extrabold mb-3">
            Crafting Timeless Simplicity
          </h3>
          <p className="text-gray-600 mb-5">
            The Entry Level line is a celebration of understated excellence —
            blending refined craftsmanship with accessible sophistication for 
